...
namespace ParallelImageEditorV2
{
    public partial class ParallelImageEditorV2 : Form
    {
        ...
        private async Task<Bitmap> ApplyBlackAndWhiteFilterAsync(Image image)
        {
            return await Task.Run(() =>
            {
                ...
                var allPixels = Enumerable.Range(0, image.Width)
                                                .SelectMany(x => Enumerable.Range(0, image.Height), (x, y) => new { x, y });

                allPixels.AsParallel().ForAll(pixel =>
                {
                    Color pixelColor;
                    lock (imageLock)
                    {
                        pixelColor = filteredImage.GetPixel(pixel.x, pixel.y);
                    }
                    ...
                    lock (imageLock)
                    {
                        filteredImage.SetPixel(pixel.x, pixel.y, newColor);
                    }
                });
